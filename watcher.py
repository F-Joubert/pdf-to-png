            ...
            if os.path.isfile(output_file):
                if os.path.isfile(abs_path):
                    ...
                    if settings_dict["Log Deletes"]:
                        add_event_to_database_table(f"{datetime.today()}", "Delete", fr"Delete source pdf: {abs_path}", "logs")
                
    except Exception as err:
        print(f"Watcher error: {err}")
        if settings_dict["Log Errors"]:
            add_event_to_database_table(f"{datetime.today()}", "Error", fr"Failed to Convert {event.src_path} - Error: {err}", "logs")

def on_deleted(event):
    try:
        if settings_dict["Log Deletes"]:
            print(fr"{event.src_path} has been deleted.")
...
def on_modified(event):
    try:
        if settings_dict["Log Modifies"]:
            ...
            add_event_to_database_table(f"{datetime.today()}", "Modify", fr"{event.src_path} has been modified.", "logs")
    except Exception as e:
...
def on_moved(event):
    try:
        if settings_dict["Log Moves"]:
            print(fr"{event.src_path} has been moved to {event.dest_path}.")
            add_event_to_database_table(f"{datetime.today()}", "Modify", fr"{event.src_path} has been moved to {event.dest_path}.", "logs")
    except Exception as e:
